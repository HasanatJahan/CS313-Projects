...
public class StatePopulations {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int numZipCode = Integer.parseInt(String.valueOf(myZipCode));
        String stateAbbrev = "";
        for (i = 0; i < zipCodeState.size(); ++i) {
            // TODO: Using ZIP code, find state abbreviation
            int arrayVal = zipCodeState.get(i).getKey();
            if(arrayVal == numZipCode){
                stateAbbrev = zipCodeState.get(i).getVal();
            }
        }
        ...
        for (i = 0; i < abbrevState.size(); ++i) {
            ...
            String abbrevName = abbrevState.get(i).getKey();
            if(abbrevName.equalsIgnoreCase(stateAbbrev)){
                numStrName = abbrevState.get(i).getVal();
            }
        }
        int stateNumPop = 0;
        int matchingIndex = 0;
        for (i = 0; i < statePopulation.size(); ++i) {
            ...
            String stateName = statePopulation.get(i).getKey();
            if(stateName.equalsIgnoreCase(numStrName)){
                matchingIndex = i;
//                stateNumPop = statePopulation.get(i).getVal();
            }
        }

        statePopulation.get(matchingIndex).printInfo();
    }
}